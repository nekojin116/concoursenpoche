...
class Helper {
	...
	public static function get_attachments( $args ) {
		...
		foreach ( $posts as $post ) {
			...
			$download_count = self::get_download_count($post->ID);
			$file  = array(
				'document_id' => $post->ID,
	...
	public static function render_row( $columns, $file, $attributes ) {
		...
		foreach ( $columns as $column ) {
			...
			if ('counter' === $column['key']) {
				?>
					<td>
						<div class="cf-download-count"><?php echo esc_html($file['download_count']) ?></div>
					</td>
				<?php
			}
				...
				?>
					<td>
						<div class="cf-column-last">
							...
							<a download rel="noopener noreferrer" href="<?php echo esc_url( $file['link'] ); ?>" class="btn-download" data-document-id="<?php echo esc_attr($file['document_id']) ?>">
	...
	public static function register_localize_script($args){
		wp_localize_script(
			'catf-dg-datatables',
			'catf_dg',
			$args
